...
<?php foreach($items as $i): ?><tr><td><?=e($i['name'])?></td><td><?=e($i['category'])?></td><td>Rs. <?=number_format($i['price'],2)?></td><td><?=e($i['status'])?></td><td><a href="item_edit.php?id=<?=$i['id']?>">Edit</a> | <a class="danger-link" data-confirm="Delete this item?" href="item_delete.php?id=<?=$i['id']?>">Delete</a></td></tr><?php endforeach; ?></table></div></section>
...
<?php foreach($users as $u): ?><tr><td><?=$u['id']?></td><td><?=e($u['username'])?></td><td><?=e($u['email'])?></td><td><?=e($u['role'])?></td><td><?php if($u['role']!=='admin'): ?><a class="danger-link" data-confirm="Delete this user?" href="user_delete.php?id=<?=$u['id']?>">Delete</a><?php endif;?></td></tr><?php endforeach; ?></table></div></section>
</main><script src="../js/script.js"></script><?php include '../includes/footer.php'; ?></body></html>